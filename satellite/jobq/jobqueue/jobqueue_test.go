...
func TestJobqueueRetry(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		for i := range jobs {
			...
			jobs[i].ID.Position = uint64(testrand.Int63n(1 << 62))
			jobs[i].Health = testrand.Float64n(1000) / 1000.0
			jobs[i].Placement = 42
			jobs[i].NumNormalizedHealthy = int16(testrand.Intn(80))
			jobs[i].NumNormalizedRetrievable = int16(testrand.Intn(80))
			jobs[i].NumOutOfPlacement = int16(testrand.Intn(80))
		...
		secondRetryJob := jobq.RepairJob{
			ID:              jobq.SegmentIdentifier{StreamID: testrand.UUID(), Position: uint64(testrand.Int63n(1 << 62))},
